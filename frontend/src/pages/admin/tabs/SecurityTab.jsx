...
const SecurityTab = () => {
  ...
  const fetchAuditLogs = async (page = 1, append = false) => {
    if (!append) setIsLoadingAuditLogs(true);
    ...
    try {
      ...
      if (!append) setIsLoadingAuditLogs(false);
    }
  };

  const fetchBannedIPs = async (page = 1, append = false) => {
    if (!append) setIsLoadingBannedIPs(true);
    const token = localStorage.getItem('token');
    try {
      ...
      if (!append) setIsLoadingBannedIPs(false);
    }
  };
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchBannedIPs(1, false), 500);
    ...
    <div className="animate-fade-in-up">
      ...
      <div id="audit-logs-panel" className={`mb-10 bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border border-smart-light/10 dark:border-gray-700 overflow-hidden transition-all duration-300 ${isAuditLogsExpanded ? 'h-auto max-h-[800px] flex flex-col' : ''}`}>
        <div className="bg-smart-bg dark:bg-gray-900 px-8 py-6 border-b border-smart-light/10 flex justify-between items-center cursor-pointer hover:bg-smart-bg/80 dark:hover:bg-gray-800 transition-colors" onClick={() => setIsAuditLogsExpanded(!isAuditLogsExpanded)}>
          ...
          <div className="flex items-center text-smart-gray dark:text-gray-400">
            <button onClick={(e) => { e.stopPropagation(); handleClearAuditLogs(30); }} className="hidden sm:flex items-center mr-2 px-3 py-1.5 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors border border-yellow-500/20" disabled={auditLogs.length === 0}>
            ...
            <button onClick={(e) => { e.stopPropagation(); handleClearAuditLogs(null); }} className="hidden sm:flex items-center mr-2 px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors border border-red-500/20" disabled={auditLogs.length === 0}>
            ...
            <button onClick={(e) => { e.stopPropagation(); handleExportAuditLogsCSV(); }} className="hidden sm:flex items-center mr-4 px-3 py-1.5 bg-smart-light/10 hover:bg-smart-light/20 text-smart-light rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors border border-smart-light/20" disabled={auditLogs.length === 0}>
          ...
          <div className="overflow-y-auto overflow-x-auto flex-grow">
              ...
              <tbody className="divide-y divide-smart-bg dark:divide-gray-700">
                {auditLogs.map(log => (
                  <tr key={log._id} className="hover:bg-smart-bg/50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-4 py-3 pl-6 text-[11px] font-bold text-smart-gray dark:text-gray-400">{new Date(log.createdAt).toLocaleString()}</td>
            ...
            {auditLogHasMore && (
              <div className="flex justify-center p-4 bg-smart-bg dark:bg-gray-900 border-t border-smart-light/10">
                <button onClick={handleLoadMoreAuditLogs} disabled={isLoadingAuditLogs} className="px-6 py-2.5 bg-smart-light/10 hover:bg-smart-light/20 text-smart-dark dark:text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition-colors border border-smart-light/20">
      ...
      <div id="banned-ips-panel" className={`mb-10 bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border border-smart-light/10 dark:border-gray-700 overflow-hidden transition-all duration-300 ${isBannedIPsExpanded ? 'h-auto max-h-[800px] flex flex-col' : ''}`}>
        <div className="bg-smart-bg dark:bg-gray-900 px-8 py-6 border-b border-smart-light/10 flex justify-between items-center cursor-pointer hover:bg-smart-bg/80 dark:hover:bg-gray-800 transition-colors" onClick={() => setIsBannedIPsExpanded(!isBannedIPsExpanded)}>
          ...
          <div className="flex items-center text-smart-gray dark:text-gray-400">
            <button onClick={(e) => { e.stopPropagation(); handleExportBannedIPsCSV(); }} className="hidden sm:flex items-center mr-4 px-3 py-1.5 bg-smart-light/10 hover:bg-smart-light/20 text-smart-light rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors border border-smart-light/20" disabled={totalBannedIPs === 0}>
              <svg className="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
              Export CSV
            </button>
          ...
          <div className="flex flex-col h-full overflow-hidden">
            <div className="bg-smart-bg/30 dark:bg-gray-900/30 px-8 py-5 border-b border-smart-light/10 flex flex-col md:flex-row gap-4 justify-between items-center">
              <div className="relative w-full md:max-w-md">
                <input type="text" placeholder="SEARCH BY IP OR REASON..." value={bannedIPsSearchQuery} onChange={(e) => setBannedIPsSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-2xl border-2 border-smart-light/20 bg-white dark:bg-gray-800 text-smart-dark dark:text-white focus:ring-4 focus:ring-smart-light/20 focus:border-smart-light outline-none transition font-mono text-xs font-black tracking-widest" />
            ...
            <div className="overflow-y-auto overflow-x-auto flex-grow">
              ...
              {bannedIPsHasMore && (
                <div className="flex justify-center p-4 bg-smart-bg dark:bg-gray-900 border-t border-smart-light/10">
                  <button onClick={handleLoadMoreBannedIPs} disabled={isLoadingBannedIPs} className="px-6 py-2.5 bg-smart-light/10 hover:bg-smart-light/20 text-smart-dark dark:text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition-colors border border-smart-light/20">
